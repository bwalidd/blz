...
std::string initialisation_param(std::string &string_to_pars,std::string parame)
{
    int position = 0 ,position_end = 0;
    std::string line;
...
one_server::one_server(std::string string_to_pars)
{
    ...
    _server_name = initialisation_param(string_to_pars,"server_name");
    listen = std::stoi(initialisation_param(string_to_pars,"listen"));
    client_body_size =  initialisation_param(string_to_pars,"client_max_body_size");             
    _Host = initialisation_param(string_to_pars,"host");
    int i =0, j;
    position = 0 ;
    position_end = 0;
    while (position != std::string::npos && position_end != std::string::npos)
...
Location::Location(std::string location)
{
    ...
    _root = initialisation_param(location,"root");
    _autoindex = 0;
    _upload= 0;
    if (initialisation_param(location,"autoindex") == "on")
        _autoindex = 1;
    if (initialisation_param(location,"upload") == "on")
        _upload= 1;
